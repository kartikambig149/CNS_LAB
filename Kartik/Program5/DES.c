...
void main()
{
int i,j,index;int k1[8], k2[8];
int afterp10[11], ls1[10], ls2[10], afterip[8], afterep[8],
afterp4[4];
...
for(j=0; j<2; j++)
aftersboxesone[j] = s0_binary[j];for(i=0,j=2; i<2,j<4; i++,j++)
...
afterone[j] = afterp4[i];afterone[j] = '\0';
printf("\nAfter first part= ");
for(i=0; i<8; i++)
printf("%d ", afterone[i]);
for(j=0; j<4; j++)
...
for(i=0; i<8; i++)
k2[i] = k2[i] ^ afterep[i];
printf("\n\nAfter XOR operation with 2nd Key= ");for(i=0; i<8; i++)
printf("%d ", k2[i]);
...
printf("\n\nAfter XOR operation with left nibble of after firstpart= ");
for(i=0; i<4; i++)
printf("%d ", afterp4[i]);
for(i=0; i<4; i++)
